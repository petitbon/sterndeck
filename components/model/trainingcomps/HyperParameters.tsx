  ...
  return (
    <>
      <div className="pb-2 flex w-full">
        <div className="w-1/5 p-3 mx-4">
          <BaseModel user_uid={user_uid} model_id={modelState.id} baseModel={modelState.model} />
        </div>
        <div className="w-1/5 p-3 mx-4">
          <Epochs user_uid={user_uid} model_id={modelState.id} n_epochs={modelState.hyperparams?.n_epochs} />
        </div>
        <div className="w-1/5 p-3 mx-4">
          <Batch user_uid={user_uid} model_id={modelState.id} batch={modelState.hyperparams?.batch_size} />
        </div>
        <div className="w-1/5 p-3 mx-4">
          <LearningRate user_uid={user_uid} model_id={modelState.id} learning_rate={modelState.hyperparams?.learning_rate_multiplier} />
        </div>
        <div className="w-1/5 p-3 mx-4">
          <PromptLoss user_uid={user_uid} model_id={modelState.id} prompt_loss_weight={modelState.hyperparams?.prompt_loss_weight} />
        </div>
      </div>
    </>
  );
