...
export default function FineTune({ user, model, training_file_id, fine_tune }: Props) {
  ...
  const cancelTraining = async (fine_tune_id: string) => {
    ...
    try {
      ...
      console.error('Error canceling fine-tune:', error);
    }
  };
        ...
        <div className="flex w-full">
          {fine_tune.fine_tuned_model ? (
            <div className="flex flex-col">
              <div className="text-sm font-semibold">{fine_tune?.fine_tuned_model}</div>
            ...
            <div className="flex items-center">
              <div className="text-sm">Status: {fine_tune.events[fine_tune.events.length - 1].message} </div>
