...
import { logOut } from '@lib/firebase/authentication';
import React, { useContext, useEffect, useRef, useState } from 'react';
import { getMoreTasks, getTasks } from '@lib/firebase/task.firestore';
...
function Todos() {
  ...
  useEffect(() => {
    if (isVisible === true && !endOfTask && todos.length > 0) {
      ...
      if (tab.title === TAB_IN_PROGRESS.title) {
        getMoreTasks(todos[todos.length - 1].id, false, user.uid).then((tasks) => {
          if (tasks.length < LIMIT_DATA_PER_REQUEST) {
            setEndOfTask(true);
          }
        ...
        getMoreTasks(todos[todos.length - 1].id, true, user.uid).then((tasks) => {
          if (tasks.length < LIMIT_DATA_PER_REQUEST) {
            setEndOfTask(true);
          }
          setTodos([...todos, ...tasks]);
  ...
  return (
    <div className="flex flex-col h-full md:w-1/2 md:m-auto p-4 text-white">
      <i className="bi bi-arrow-up-right-square-fill self-end text-2xl cursor-pointer" onClick={Logout}></i>
      ...
      <div className="flex justify-between mt-3 mx-6 py-2 border-b cursor-pointer">
        <p className={tab.title === TAB_IN_PROGRESS.title ? `mr-10 px-4 ${activeStyle}` : 'mr-10 px-4'} onClick={() => changeTab(TAB_IN_PROGRESS.title)}>
          {TAB_IN_PROGRESS.title}
        </p>
        <p className={tab.title === TAB_DONE.title ? `px-4 ${activeStyle}` : 'px-4'} onClick={() => changeTab(TAB_DONE.title)}>
          {TAB_DONE.title}
        </p>
      </div>
        ...
        {todos.length > 0 && todosLoading === false && (
          <div ref={ref} className="px-6 pt-3">
            ...
            <Link href="/todos/newtask">
              ...
              <span className="capitalize">Add new task HERE</span>
            </Link>
          </div>
        )}
