...
export default function ExpenseDialog(props) {
  ...
  const updateFormField = (event, field) => {
    setFormFields((previousState) => ({ ...previousState, [field]: event.target.value }));
  };

  // Set the relevant fields for receipt image
  const setFileData = (target) => {
    const file = target.files[0];
    setFormFields((previousState) => ({ ...previousState, fileName: file.name }));
    setFormFields((previousState) => ({ ...previousState, file }));
  };
        ...
        <Stack>
          <LocalizationProvider dateAdapter={AdapterDateFns}>
            <DatePicker
              ...
              onChange={(newDate) => {
                setFormFields((previousState) => ({ ...previousState, date: newDate }));
              }}
              maxDate={new Date()}
              renderInput={(parameters) => <TextField color="tertiary" {...parameters} />}
            />
